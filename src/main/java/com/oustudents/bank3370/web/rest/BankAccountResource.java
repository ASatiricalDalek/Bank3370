...
public class BankAccountResource {
    ...
    public List<BankAccount> getAllBankAccounts(@RequestParam(required = false) String filter) {
        if ("transactionlog-is-null".equals(filter)) {
            log.debug("REST request to get all BankAccounts where transactionLog is null");
            return StreamSupport
                .stream(bankAccountRepository.findAll().spliterator(), false)
                .filter(bankAccount -> bankAccount.getTransactionLog() == null)
