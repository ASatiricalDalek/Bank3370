...
export class PatronResolve implements Resolve<IPatron> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<Patron> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<Patron>) => response.ok),
                map((patron: HttpResponse<Patron>) => patron.body)
            );
        }
